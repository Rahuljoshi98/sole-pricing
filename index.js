...
(function () {
  ...
  const industries = [
    ...
    "Retail",
    "Trades",
    "Transport, Delivery & People Movers",
    "Wholesale",
    "Other",
    ...
    "Agriculture",
    "Cleaning Services",
    "Hairdressers (rent a chair)",
    "Contractors & Professional Services",
    "Property Management",
    "eCommerce",
    "General Business",
    "Hair & Beauty (Makeup Artists / Beauty Therapists)",
    "Hospitality (Catering / Food Truck / Barista / Chef)",
    "Consulting (Business / Strategy / Specialist Advisory)",
    ...
    "Transport (Owner-driver / Courier)",
    "Wholesaler (Independent Distributor / Agent)",
  ];
  ...
  function renderChips() {
    ...
    const filtered = industries.filter((i) => i.toLowerCase().includes(query));
    ...
    const html = visible
      .map((item) => {
        const selected = state.selectedIndustries.has(item);
        return `<button
        class="chip${selected ? " selected" : ""}"
        data-industry="${item}"
        aria-pressed="${selected}"
      >${item}${selected ? chipCheckSVG() : ""}</button>`;
      })
      .join("");
